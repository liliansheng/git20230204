                ...
                <div>
                    <ul class="nav nav-pills nav-stacked" id="typeUl">
                        <c:forEach items="${typeInfo}" var="item">
                            <li id="li_${item.typeId}"><a href="index?actionName=searchTypeId&typeId=${item.typeId}"><span id="sp_${item.typeId}">${item.groupName}</span> <span class="badge">${item.noteCount}</span></a></li>
                        </c:forEach>
                    </ul>
                </div>
...
<div class="container">
    ...
    &lt;%&ndash; 动态包含页面 &ndash;%&gt;
    &lt;%&ndash; 云记列表 &ndash;%&gt;
    &lt;%&ndash;<jsp:include page="note/list.jsp"></jsp:include>&ndash;%&gt;
    &lt;%&ndash; 个人中心 &ndash;%&gt;
    &lt;%&ndash;<jsp:include page="user/info.jsp"></jsp:include>&ndash;%&gt;
    &lt;%&ndash; 类型列表 &ndash;%&gt;
    &lt;%&ndash;<jsp:include page="type/list.jsp"></jsp:include>&ndash;%&gt;

    &lt;%&ndash; 通过后台设置动态显示的页面，通过包含加载进来 &ndash;%&gt;
    &lt;%&ndash; 如果获取到后台设置的值，则显示；如果为获取到，则设置默认 &ndash;%&gt;
    <c:if test="${empty changePage}">
        <jsp:include page="note/list.jsp"></jsp:include>
    </c:if>
    <c:if test="${!empty changePage}">
